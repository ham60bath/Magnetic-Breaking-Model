...
a=250.026;b=-0.039;c= -33.273;

def test(x):
    return a*np.exp(b*x)+c

data = pd.read_excel('C:/Users/harry/OneDrive/Documents/Uni/Y2/S2/Labs/Al graphs/AL_0_attempt1.xlsx' , 
                        names=('time', 'w'), 
                        usecols=(0,1), 
                        nrows = 1000)
...
while(i<862):
    val=data.w[i]
    t=data.time[i]
    if(val-30<test(t) and test(t)<val+7):
        newt.append(t)
        neww.append(val)
        
    i=i+1
plt.plot(data.time, test(data.time), color = 'green')
...
i= 0.170; Tc = 0.01 ;
sig=42.3*(10**6);R=0.25;S=1.01*(10**-3);d=2*(10**-3); B = 210 *10**-3
